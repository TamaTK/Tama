<?php
   ...
   function getProducts($searchTerm = null) {
      ...
      if(!$searchTerm) {
         ...
         $sql ='SELECT * FROM PRODUCTS WHERE PRICE LIKE "'.$searchTerm.'" OR PRODUCTNAME LIKE "'.$searchTerm.'" OR DESCRIPTION LIKE "'.$searchTerm.'" OR INSERT INTO PRODUCTS (PRICE, PRODUCTNAME, DESCRIPTION, NUTRITIONAL) 
 LIKE "'.$searchTerm.'"';
      }
      $ret = $db->query($sql);
      $array = [];
